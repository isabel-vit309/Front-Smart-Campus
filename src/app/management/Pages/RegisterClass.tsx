...
export default function RegisterClass() {
  ...
  useEffect(() => {
    Promise.all([api.get('/api/courses'), api.get('/api/rooms')]).then(([c, r]) => {
      setCourses(c.data);
      setRooms(r.data);
    }).catch(() => {});
  }, []);
  ...
  const resetForm = () => {
    setSelectedCourseId(''); setSelectedRoomId('');
    setSelectedDay(null); setStartTime(''); setEndTime('');
  };

  const handleRegister = async () => {
    if (!selectedCourseId || !selectedRoomId || selectedDay === null || !startTime || !endTime) {
      Alert.alert('Erro', 'Selecione curso, sala, dia e preencha os horários.'); return;
    }
    const timeRegex = /^([01]\d|2[0-3]):[0-5]\d$/;
    if (!timeRegex.test(startTime) || !timeRegex.test(endTime)) {
      Alert.alert('Erro', 'Horário inválido. Use o formato HH:MM (ex: 18:00).'); return;
    }
    if (startTime >= endTime) {
      Alert.alert('Erro', 'O horário de início deve ser antes do término.'); return;
    }
    setLoading(true);
    try {
      ...
      setLastRegistered({ course: course?.name, room: room?.name, startTime, endTime });
      ...
      <SafeAreaView style={styles.content}>
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>Registre aqui as aulas, horários e salas de cada turma</Text>
          </View>

          <View style={styles.card}>
            <Text style={styles.titleCard}>REGISTRO DE AULA</Text>
            <Text style={styles.cardSubtitle}>Preencha as informações para registrar uma aula</Text>
            ...
            <View style={styles.chipGrid}>
              {courses.map((c) => (
                ...
                >
                  <Text style={[styles.chipText, selectedCourseId === c.id && { color: "#FFF" }]}>{c.name}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={styles.chipGrid}>
              {rooms.map((r) => (
                ...
                >
                  <Text style={[styles.chipText, selectedRoomId === r.id && { color: "#FFF" }]}>{r.name}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={styles.chipGrid}>
              {DAYS.map((d) => (
                <TouchableOpacity
                  key={d.value}
                  style={[styles.chip, selectedDay === d.value && styles.chipActive]}
                  onPress={() => setSelectedDay(d.value)}
                >
                  <Text style={[styles.chipText, selectedDay === d.value && { color: "#FFF" }]}>{d.label}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={styles.row}>
              <View style={styles.halfField}>
                ...
                <TextInput
                  style={styles.input} placeholder="Ex: 18:00" placeholderTextColor="#6B8BB0"
              ...
              <View style={styles.halfField}>
                ...
                <TextInput
                  style={styles.input} placeholder="Ex: 20:00" placeholderTextColor="#6B8BB0"
            ...
            <View style={styles.card}>
              ...
              <View style={styles.recentCard}>
                ...
                <View style={styles.recentInfo}>
                  <Text style={styles.recentDiscipline}>{lastRegistered.course}</Text>
                  <Text style={styles.recentTime}>{lastRegistered.startTime} - {lastRegistered.endTime}</Text>
...
const styles = StyleSheet.create({
  ...
  recentCard: {
    backgroundColor: "#0F213F", width: "100%", marginTop: 12,
    borderRadius: 12, padding: 12, flexDirection: "row", alignItems: "center",
  },
  recentIconBox: {
    width: 50, height: 50, borderRadius: 8, backgroundColor: "#0B4BBB",
    alignItems: "center", justifyContent: "center", marginRight: 14,
  },
  recentInfo: { flex: 1 },
  recentDiscipline: { color: "#FFFFFF", fontSize: 15, fontWeight: "bold" },
  recentTime: { color: "#2F6BFF", fontSize: 13, marginTop: 4, fontWeight: "500" },
