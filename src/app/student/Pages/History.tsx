...
const HistoryCard = ({ log }: { log: AccessLog }) => {
  ...
  const dayName = entered
    .toLocaleDateString("pt-BR", { weekday: "long" })
    .toUpperCase();
  const courseName = log.course?.name ?? "Disciplina";
  const timeIn = entered.toLocaleTimeString("pt-BR", {
    hour: "2-digit",
    minute: "2-digit",
  });
  const timeOut = exited
    ? exited.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })
    : "--:--";
...
export default function History() {
  ...
  const todayLogs = logs.filter(
    (l) => new Date(l.enteredAt).toDateString() === todayStr,
  );
  const pastLogs = logs.filter(
    (l) => new Date(l.enteredAt).toDateString() !== todayStr,
  );
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.card}>
            ...
            {pastLogs.length === 0 ? (
              ...
              pastLogs
                .slice(0, 20)
                .map((log) => <HistoryCard key={log.id} log={log} />)
            )}
          </View>
        </ScrollView>
      </SafeAreaView>
